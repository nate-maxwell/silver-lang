...
| `seed(value)`       | Seed the generator with an integer and return null.                         |
...
`randint` raises `ValueError` when `a` is greater than `b`. Map iteration order is unspecified, so seeded calls to
`randkey` are repeatable as random index choices but not guaranteed to select the same key across separate runs.
