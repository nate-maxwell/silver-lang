...
// truthiness rules as values created directly by the evaluator.
func New(out io.Writer, null *object.Null) *Registry {
	if out == nil {
		out = io.Discard
	}
	if null == nil {
...
func newRegistry(groups ...[]definition) *Registry {
	registry := &Registry{values: make(map[string]*object.Builtin)}
	for _, group := range groups {
		for _, builtin := range group {
			...
			registry.values[builtin.name] = &object.Builtin{Fn: builtin.fn}
		}
	}
	return registry
}
