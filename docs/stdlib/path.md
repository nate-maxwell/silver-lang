...
A `Path` exposes `path`, `anchor`, `drive`, `name`, `root`, `stem`, and `suffix` strings, plus `parts` and `suffixes`
arrays. These properties are computed when the value is constructed.
